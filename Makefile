...
BUILD		:=	build
SOURCES		:=	source shaders include/external
DATA		:=	data
INCLUDES	:=	include
GRAPHICS	:=	gfx
#GFXBUILD	:=	$(BUILD)
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
APP_TITLE   := Minecraft Skin Viewer
APP_DESCRIPTION := A simple application to view Minecraft skins on your 3DS
...
CFLAGS	+=	$(INCLUDE) -D__3DS__ -DTINYGLTF_USE_CPP14
...
cia:
	...
	@bannertool makebanner -i gfx/banner.png -a gfx/banner.wav -o banner.bnr
	@makerom -f cia -rsf 3ds.rsf -elf $(TARGET)-strip.elf -icon $(OUTPUT).smdh -banner banner.bnr -o $(TARGET).cia
